...
def train():
    ...
    for iteration in tqdm(range(20)):
        # Define the model
        model = CLIPFinetune(num_classes, class_tokens, device, True).to(device)
...
dataModule = DataModule(train_path,
                        ...
                        batch_size,
                        num_workers)
with open(convnext_data_path, "rb") as file:
    dataModule.dataset_list, dataModule.unlabelled_dataset, dataModule.pseudo_labelled_dataset = pickle.load(file)
